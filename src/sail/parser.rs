...
fn read_value(
    ...
    match c {
        ...
        b'"' => {
            chars.next();
            value = read_string(chars, acc, tbl, elt)?;
            acc.clear();
        }
        b'#' => {
            chars.next();
            value = read_special(chars, acc, tbl, elt)?;
            acc.clear();
        }
        b'+' | b'-' => {
            acc.push(chars.next().unwrap());
            if chars.peek().unwrap().is_ascii_digit() {
                value = read_number(chars, acc, tbl, elt)?;
            } else {
...
fn read_quote(
    ...
    unsafe {
        let head = super::init_ref(memmgt::which_mem_sector(tbl), elt, super::SlRefMode::List);
        let start = super::init_symbol(
            memmgt::which_mem_sector(tbl),
            true,
            super::SlSymbolMode::ById,
            0,
        );
...
fn read_list(
    ...
    let head =
        unsafe { super::init_ref(memmgt::which_mem_sector(tbl), elt, super::SlRefMode::List) };
...
fn read_string(
    ...
    let string = unsafe { super::init_string(memmgt::which_mem_sector(tbl), elt, 32) };
...
fn read_number(
    ...
    process_num(unsafe { str::from_utf8_unchecked(acc) }, tbl, elt)
}
...
fn read_special(
    ...
    let val = unsafe { super::init_bool(memmgt::which_mem_sector(tbl), elt) };
...
fn process_num(slice: &str, tbl: *mut SlHead, elt: bool) -> Result<*mut SlHead, SailErr> {
    let val;
    if let Ok(n) = slice.parse::<i64>() {
        unsafe {
            val = super::init_fixint(memmgt::which_mem_sector(tbl), elt);
            super::fixint_set(val, n);
        }
        return Ok(val);
    } else if let Ok(n) = slice.parse::<f64>() {
        unsafe {
            val = super::init_fixfloat(memmgt::which_mem_sector(tbl), elt);
